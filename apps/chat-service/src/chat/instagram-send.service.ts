 ...
 */
@Injectable()
export class InstagramSendService implements ChannelSendService {
  ...
  /**
   * ChannelSendService interface method — delegates to sendImageMessage.
   */
  async sendMediaMessage(recipient: string, mediaUrl: string, _caption?: string): Promise<void> {
    return this.sendImageMessage(recipient, mediaUrl);
  }
}
